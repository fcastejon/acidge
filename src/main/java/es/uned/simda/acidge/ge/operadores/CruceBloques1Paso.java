...
*/
/*
 * Clase que implementa el cruce por bloques con un paso 
 * 
...
public class CruceBloques1Paso extends OperadorRecombinacion 
{
	private final static Logger logger = Logger.getLogger(CruceBloques1Paso.class.getName());

	int maxPunto, minPunto;
	
	public CruceBloques1Paso(GEProperties geproperties, RandomGenerator randomGenerator)
	{
	...
	public CruceBloques1Paso(int CrossoverType, double CrossoverRate, 
	...
	public Genotipo cruza(Genotipo padre1, Genotipo padre2)
	{
		...
		if(randomGenerator.random() <= CrossoverRate)
			...
			int punto1 = randomGenerator.randomInt(genes1.length / CrossoverBlockSize) * CrossoverBlockSize;
			int punto2 = randomGenerator.randomInt(genes2.length / CrossoverBlockSize) * CrossoverBlockSize;
